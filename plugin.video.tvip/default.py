...
Guide = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.tvip/resources/catchup', 'guide.xml'))
GuideLoc = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.tvip/resources/catchup', 'g'))
...
def start():
	if username=="":
		...
		if auth == "":
			...
			xbmcgui.Dialog().ok('Tvip', line1, line2, line3)
			addonsettings('ADS2','')
			xbmc.executebuiltin('Container.Refresh')
			home()
	else:
		...
		auth = tools.OPEN_URL(auth)
		if not auth=="":
			tools.addDir('[B][COLOR white]Detalles de la cuenta[/COLOR][/B]','url',6,accounticon,fanart,'')
			...
			tools.addDir('[B][COLOR white]Configuraciones[/COLOR][/B]','url',16,settingsicon,fanart,'')
			tools.addDir('[B][COLOR white]Eliminar la cuenta[/COLOR][/B]','LO',10,logouticon,fanart,'')
...
def search_scat(url):
    text = searchdialog()
    if not text:
        xbmc.executebuiltin("Notification([COLOR orange][B]Nada encontrado o mal escrito el titulo[/B][/COLOR],Borrado,4000,"+icon+")")
        return
...
def subm():
	#tools.addDir('[COLOR white]Cargar la lista completa[/COLOR]','vod',333,vodicon,fanart,'')
...
def search():
    ...
    if not text:
        xbmc.executebuiltin("Notification([COLOR gold][B]Nada encontrado o mal escrito el titulo[/B][/COLOR],Borrado,4000,"+icon+")")
        return
    xbmc.log(str(text))
...
def addonsettings(url,description):
	if   url =="CC":
		...
		xbmc.executebuiltin('Runscript("special://home/addons/plugin.video.tvip/resources/modules/speedtest.py")')
	elif url =="META":
...
def userpopup():
	...
	kb.setHeading('Escriba el nombre de usuario')
...
def passpopup():
	kb =xbmc.Keyboard ('', 'heading', True)
	kb.setHeading('Escriba la clave de acceso')
	kb.setHiddenInput(False)
	kb.doModal()
	if (kb.isConfirmed()):
		text = kb.getText()
